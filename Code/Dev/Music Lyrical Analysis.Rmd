...
# Narrative, watch out for the usage of I and we
# Ngrams: Bi and Tri grams. "time has come" 10, "has come" 6, in a better ngram analysis, parts are not recounted
#         recounting the parts skews this type of analysis
#         tidytext ngrams have this shortcoming, tidytext performs blind bi-grams. We can look to use the TM package instead.

# WORKFLOW code from BK ======
# Corpus -> TidyText 
#MyCorpus <- Corpus(DataframeSource(wrk.validclaims)) 
#MyCorpusQ <- quanteda::corpus(MyCorpus) 
#tidy.data <- tidy(MyCorpusQ) 
#Bigram setup ======
#NgramTokenizer <- function(x) {RWeka::NGramTokenizer(x, RWeka::Weka_control(min = 1, max = 2))} 
#MyCorpus <- Corpus(DataframeSource(wrk.validclaims)) 
# Create DTM ========
#  dtm <<- DocumentTermMatrix(MyCorpus, control = list(tokenize = NgramTokenizer , 
#    weighting = weightTf, wordLengths = c(3, 60), bounds = list(global = c(5,Inf)))) 
#
#
#
# Convert some of the long tables from kable to paginated DT tables, if appropriate

# Addressing the so what?
# From dad: your coloured word count chart. What density of words corresponds to each coloured dot? 
# 0. its a clinical dissection of lyrics. Why is it we associate with a song, a melody or rhythm? Why does that piece make an impact upon us when other music has no charm? 
# We can imagine or emphasize with the words, or the melody takes us away from our reality to a place where stress is lessened or removed for awhile. Medical studies have shown that music does have a beneficial effect on us.   As I mentioned earlier: it’s the good vibrations, that matter.
#There is much to analyse  if you consider rhythm, melody, instruments, the arrangement of music and whether the words match the music, how are #the words arranged, where each instrument comes into the piece and does it suit the melody or distract from it. 

#
# 1. Having never listened to Iron Maiden's Powerslave, I now have a much clearer understanding of this album's content
# 2. In consultation with my family, I selected (cherry picked) 6 albums we percieved to be "eclectic" from one another, does this analysis agree with this perception? <<weave in this throughout the analysis, each component accept/rejecting the idea and come to a conclusion at the end.
# 3. Exploration of copy write legality. in forensic analysis to prove or disprove copy write infringements.
# 4. How could this analysis be used on concert music,jazz, chamber music or modern music that does not have words?
...
More recently, this thought pattern commentary has been circulating in my head while listening to my personal collection of music:
  ...
  1.	“This is such an eclectic playlist, **how do you make sense** of such a diverse collection of music?”
...
Working purely with the lyrical components for a selected collection of music album data, I will apply contemporary **text mining** and **natural language processing** techniques to this data to explore the above questions, and build insights.
 ...
 * Seemingly larger sized word cloud with more defined words of Elton John versus a smaller word cloud with less defined words in Daft Punk.
 * Perceived similar, synonymous word usage between U2 and Killswitch Engage: "heart", "eyes". "life" versus "love".
...
```{r "Exploration_BiGrams_Network", echo=FALSE, eval=TRUE, fig.width=7, fig.height=7, fig.align='center'}
...
Interesting that no other songs had a high enough correlation in the pairwise calculation to be considered as _"significant"_.
...
**Can we make meaningful sense of the data?**
Yes! Across the selected 6 music albums and 59 individual songs identified:
...
**Text mining, NLP and Machine Learning with Music Lyrics**
  ...
  [Prince analysis - NLP](https://www.datacamp.com/community/tutorials/R-nlp-machine-learning)

  [Prince analysis - Sentments](https://www.datacamp.com/community/tutorials/sentiment-analysis-R)

  [The Ramones](http://r-blog.salvaggio.net/?p=521)

  [Rick and Morty](http://tamaszilagyi.com/blog/a-tidy-text-analysis-of-rick-and-morty/)

  [50 Years of Pop Music Lyrics](https://github.com/walkerkq/musiclyrics)

  [Radiohead & Using the Spotify API](http://www.rcharlie.com/post/fitter-happier/)

  [Alternative sentiment analysis: Using the “gloom” index to find depressing songs](http://blog.revolutionanalytics.com/2017/02/finding-radioheads-most-depressing-song-with-r.html)

  [Alternative visualisations: Visualising songs as matrix structures and find repetitions](http://giorasimchoni.com/2017/10/16/2017-10-16-repeat-yourself-the-songsim-package/)

  [Topic Modeling of Sherlock Holmes Stories](https://juliasilge.com/blog/sherlock-holmes-stm/)

  [Topic modeling using STM](http://thomaselliott.me/pdfs/earl/topic_modeling.html)
